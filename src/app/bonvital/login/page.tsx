...
export default function BonvitalLogin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/bonvital-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password }),
      });
      if (res.ok) {
        router.push("/bonvital");
        router.refresh();
      } else {
        setError(true);
      }
    ...
    <main className="min-h-screen bg-hero-bg text-hero-fg flex items-center justify-center px-6">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <p className="font-mono text-xs text-hero-fg/45 mb-8">
          This dashboard is protected. Contact Zoli for access.
        </p>
