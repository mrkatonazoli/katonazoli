...
export default function Bp2027Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/bp2027-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password }),
      });
      if (res.ok) {
        // /2027bp is served via static rewrite — force a full navigation so
        // middleware + rewrite serve the HTML with the freshly set cookie.
        window.location.assign("/2027bp");
      } else {
        setError(true);
        setLoading(false);
      }
